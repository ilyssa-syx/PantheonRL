"""Run the fair-comparison 5-layout x 3-seed Discrete SAC matrix."""
...
from train_discrete_sac_selfplay import get_run_dir, make_run_name
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the fair-comparison 15-run Discrete SAC matrix."
    )
    parser.add_argument(
...
def completed_evaluation(run_dir: Path, episodes: int) -> bool:
    ...
    return (
        evaluation.get("algo") == "discrete_sac"
...
def model_overrides(smoke_test: bool) -> Dict[str, Any]:
    if not smoke_test:
...
def run_matrix(args: argparse.Namespace) -> int:
    ...
    train_script = script_dir / "train_discrete_sac_selfplay.py"
    evaluate_script = script_dir / "eval_discrete_sac.py"
    status_name = (
        "discrete_sac_smoke_status.json"
        if args.smoke_test
        else "discrete_sac_batch_status.json"
    )
    status_path = args.output_dir / status_name
    matrix = [("simple", 0)] if args.smoke_test else [
        (layout, seed) for layout in LAYOUTS for seed in SEEDS
    ]
    timesteps = 1_000 if args.smoke_test else args.timesteps
    overrides = model_overrides(args.smoke_test)
