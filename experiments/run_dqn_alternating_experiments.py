"""
Run the 5-layout x 3-seed x 3-exploration-fraction Alternating DQN matrix.
"""
...
from train_dqn_alternating import get_run_dir, make_run_name
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the complete 45-run Alternating DQN matrix on CPU."
    )
    parser.add_argument(
        "--output-dir", type=Path, default=Path("results/alternating_dqn")
    )
    ...
    parser.add_argument(
        "--smoke-test",
        action="store_true",
        help="Run simple/seed0/fraction0.1 with 1,000 steps per agent.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def completed_evaluation(run_dir: Path, episodes: int) -> bool:
    ...
    return (
        ...
        and len(evaluation.get("episode_sparse_returns", [])) == episodes
    )
...
def run_matrix(args: argparse.Namespace) -> int:
    ...
    train_script = script_dir / "train_dqn_alternating.py"
    evaluate_script = script_dir / "evaluate_selfplay.py"
    status_name = (
        "alternating_dqn_smoke_status.json"
        if args.smoke_test
        else "alternating_dqn_batch_status.json"
    )
        ...
        try:
            if completed_training(run_dir):
                ...
                train_command = [
                    ...
                    str(seed),
                    "--partner-seed-offset",
                    str(args.partner_seed_offset),
                    "--timesteps-per-agent",
                    str(timesteps_per_agent),
                    "--phase-timesteps",
                    str(phase_timesteps),
